...
function fillTeam(teamList, team) {
    ...
    while (cnt < teamList.length) {
        ...
        i = 0;
        if (cnt + 1 === teamList.length)
            append += "<div class='col text-center p-0'><button name='delTeam' id='" + team[cnt].id + "'  type='button' class='btn btn-danger m-3'>Supprimer</button><ul class='m-0 facet-list ui-sortable'>";
        else
            append += "<div class='col text-center p-0 mr-3'><button name='delTeam' id='" + team[cnt].id + "'  type='button' class='btn btn-danger m-3'>Supprimer</button><ul class='m-0 facet-list ui-sortable'>";
        while (i < tmp_team.length) {
...
function initDelTeam() {
    $("[name='delTeam']").click(function () {
        var id = $(this).attr('id');
        let userTmp = JSON.parse(GetCookie("UserTmp"));
        let token = userTmp['token'];
        let idTeam = {id: id};
        ...
        $.ajax({
            type: "POST",
            url: ApiUrl + "Equipe/delete.php",
            headers: {'Authorization': token},
            data: JSON.stringify(idTeam),
            dataType: "JSON",
            success: function (response) {
